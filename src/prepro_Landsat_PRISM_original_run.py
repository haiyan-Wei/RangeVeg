...
PROJECT_ROOT = r'E:\range_vegcover\original_notebooks_outputs\data'
landsat_dir = r'E:\range_vegcover\original_notebooks_outputs\outputs_original'
...
extent_all = gpd.read_file(os.path.join(PROJECT_ROOT, "vectors", "Polygons_withEEid.shp"))
...
dataset = pd.read_csv(os.path.join(PROJECT_ROOT, "tables", "dataset.csv"))
...
ppt = rasterio.open(os.path.join(landsat_dir, "PRISM", "ppt.tif"))
tmean = rasterio.open(os.path.join(landsat_dir, "PRISM", "tmean.tif"))
...
points_all = gpd.read_file(os.path.join(PROJECT_ROOT, "vectors", "Points.shp"))
...
output_dir = r'E:\range_vegcover\pyfiles\outputs\original_prepro_output'
directory_temp = os.path.join(output_dir, "prepro")
...
# - Landsat
# - PRISM
directory_final = os.path.join(output_dir, "Final_data")
os.makedirs(directory_final, exist_ok=True)
os.makedirs(os.path.join(directory_final, "Landsat"), exist_ok=True)
os.makedirs(os.path.join(directory_final, "PRISM"), exist_ok=True)
...
def prePro1(rowGdrive, year_last=year_last, month_last=month_last):
    output_dir = os.path.join(directory_temp, str(rowGdrive.PrimaryKey))
    os.makedirs(output_dir, exist_ok=True)
        
    dir_EE = os.path.join(landsat_dir, "Landsat", f'{rowGdrive.PK_EE}')
    ...
    for index, band in enumerate(bands):
        ...
        with rasterio.open(
            os.path.join(output_dir, f'{band}.tif'), 
                           'w', **ras_meta) as dst:
            dst.write(ImageOLI)
    ...
    with rasterio.open(os.path.join(output_dir, "pixel_qa.tif"), 
    ...
    dates_table1.to_csv(os.path.join(output_dir, "dates_SR.csv"), index=False)


    ############################################################

    with rasterio.open(os.path.join(output_dir, "pixel_qa.tif")) as src:
        ImageOLI_qa = src.read().astype(float)
        ras_meta_qa = src.profile

    with rasterio.open(os.path.join(output_dir, "B1.tif")) as src:
        band = src.read()
        band = np.expand_dims(band, axis=-1)
        ras_meta = src.profile
    for b in bands[1:]:
        with rasterio.open(os.path.join(output_dir, f'{b}.tif')) as src:
            band1 = src.read()
            band1 = np.expand_dims(band1, axis=-1)
            band = np.concatenate((band, band1), axis=-1)

    dates = pd.read_csv(os.path.join(output_dir, "dates_SR.csv"))
    ...
    if len(listOfDel) != 0:
        ...
        for index4, b in enumerate(bands):
            with rasterio.open(os.path.join(output_dir, f'{b}.tif'), 'w', **ras_meta) as dst:
                dst.write(band[:, :, :, index4])

        ras_meta_qa.update({"driver": "GTiff",
                            "compress":'lzw',
                            "count":len(dates.index.tolist())})
        with rasterio.open(os.path.join(output_dir, "pixel_qa.tif"), 'w', **ras_meta_qa) as dst:
    ...
    dates.to_csv(os.path.join(output_dir, "dates_SR.csv"), index=False)
...
#     #########
    ...
    with rasterio.open(os.path.join(output_dir, "B2.tif")) as src:
    ...
    with rasterio.open(os.path.join(output_dir, "B3.tif")) as src:
    ...
    with rasterio.open(os.path.join(output_dir, "B4.tif")) as src:
    ...
    with rasterio.open(os.path.join(output_dir, "B5.tif")) as src:
    ...
    with rasterio.open(os.path.join(output_dir, "B7.tif")) as src:
...
# #       ###################################
    ...
    with rasterio.open(os.path.join(output_dir, "NDVI.tif"), 'w', **B3_meta) as dst:
        dst.write(NDVI)
    ...
    with rasterio.open(os.path.join(output_dir, "EVI.tif"), 'w', **B3_meta) as dst:
        dst.write(EVI)
    ...
    with rasterio.open(os.path.join(output_dir, "NDSI.tif"), 'w', **B2_meta) as dst:
        dst.write(NDSI)
    ...
    with rasterio.open(os.path.join(output_dir, "NDMI.tif"), 'w', **B4_meta) as dst:
        dst.write(NDMI)
    ...
    with rasterio.open(os.path.join(output_dir, "SAVI.tif"), 'w', **B4_meta) as dst:
        dst.write(SAVI)
    ...
    with rasterio.open(os.path.join(output_dir, "SATVI.tif"), 'w', **B3_meta) as dst:
        dst.write(SATVI)
    ...
    with rasterio.open(os.path.join(output_dir, "MSAVI.tif"), 'w', **B3_meta) as dst:
        dst.write(MSAVI)
    ...
    with rasterio.open(os.path.join(output_dir, "NBR.tif"), 'w', **B4_meta) as dst:
        dst.write(NBR)
    ...
    with rasterio.open(os.path.join(output_dir, "BSCI.tif"), 'w', **B4_meta) as dst:
        dst.write(BSCI)
    ...
    with rasterio.open(os.path.join(output_dir, "NBR2.tif"), 'w', **B4_meta) as dst:
        dst.write(NBR2)
    ...
    with rasterio.open(os.path.join(output_dir, "NDRI.tif"), 'w', **B4_meta) as dst:
        dst.write(NDRI)
    ...
    with rasterio.open(os.path.join(output_dir, "TCb.tif"), 'w', **B4_meta) as dst:
        dst.write(TCb)
    ...
    with rasterio.open(os.path.join(output_dir, "TCg.tif"), 'w', **B4_meta) as dst:
        dst.write(TCg)
    ...
    with rasterio.open(os.path.join(output_dir, "TCw.tif"), 'w', **B4_meta) as dst:
        dst.write(TCw)
    ...
    with rasterio.open(os.path.join(output_dir, "pvi.tif"), 'w', **B4_meta) as dst:
        dst.write(pvi)
    ...
    with rasterio.open(os.path.join(output_dir, "CI.tif"), 'w', **B4_meta) as dst:
        dst.write(CI)

    DFI = (1- B7/B5) * (B3 / B4)
    DFI = np.where(np.isnan(DFI), -999, DFI)
    DFI = np.where(np.isinf(DFI), -999, DFI)
    with rasterio.open(os.path.join(output_dir, "DFI.tif"), 'w', **B4_meta) as dst:
        dst.write(DFI)
        
    B1 = np.where(np.isnan(B1), -999., B1)
    with rasterio.open(os.path.join(output_dir, "B1.tif"), 'w', **B1_meta) as dst:
        dst.write(B1)
        
    B2 = np.where(np.isnan(B2), -999., B2)
    with rasterio.open(os.path.join(output_dir, "B2.tif"), 'w', **B2_meta) as dst:
        dst.write(B2)
        
    B3 = np.where(np.isnan(B3), -999., B3)
    with rasterio.open(os.path.join(output_dir, "B3.tif"), 'w', **B3_meta) as dst:
        dst.write(B3)
        
    B4 = np.where(np.isnan(B4), -999., B4)
    with rasterio.open(os.path.join(output_dir, "B4.tif"), 'w', **B4_meta) as dst:
        dst.write(B4)
        
    B5 = np.where(np.isnan(B5), -999., B5)
    with rasterio.open(os.path.join(output_dir, "B5.tif"), 'w', **B5_meta) as dst:
        dst.write(B5)
        
    B7 = np.where(np.isnan(B7), -999., B7)
    with rasterio.open(os.path.join(output_dir, "B7.tif"), 'w', **B7_meta) as dst:
        dst.write(B7)
...
######################################################
    ...
    # shutil.rmtree(os.path.join(output_dir, str(rowGdrive.PrimaryKey)))


extent_all = extent_all[extent_all.PrimaryKey.isin(dataset.PrimaryKey)].sort_values(by='PrimaryKey')
extent_all = extent_all.iloc[:50, :]
for i in range(50):
    prePro1(extent_all.iloc[i, :])
